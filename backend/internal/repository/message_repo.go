...
func (i *MessageRepo) GetAll(chat_id int) ([]models.Message, error) {
	...
	query := fmt.Sprintf(`SELECT il.id, il.image, il.content, il.sender_id, il.type, il.created_at FROM %s il 
...
func (i *MessageRepo) GetMsgById(message_id int) (models.Message, error) {
	var input models.Message
	query := fmt.Sprintf("SELECT il.id, il.image, il.content, il.sender_id, il.type, il.created_at FROM %s il WHERE il.id = $1", messageTable)
...
func (i *MessageRepo) Update(user_id, message_id int, updatedMessage models.Message) error {
	query := fmt.Sprintf(`UPDATE %s ti SET image = $1, content = $2, sender_id = $3, type = $4, created_at = $5 FROM %s ul, %s li 
							WHERE ti.id = li.message_id AND li.chat_id = ul.chat_id AND ul.user_id = $4 AND ti.id = $6`,
		messageTable, usersChats, chatMessages)
	_, err := i.db.Exec(query, updatedMessage.Image, updatedMessage.Content, updatedMessage.SenderId,
